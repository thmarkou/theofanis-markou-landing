import { motion } from "framer-motion";
import { ExternalLink } from "lucide-react";
import { useDictionary } from "@/hooks/useDictionary";
import { OFFICE_LOCATION_MAPS } from "@/lib/maps";
import { fadeUp, stagger, VIEWPORT_ONCE } from "@/lib/motion";

/**
 * Office maps + addresses: one block per office (order matches `maps.ts`).
 * Iframes stay visible (never hidden by CSS).
 */
export function CompanyMap() {
  const { location } = useDictionary();

  return (
    <section
      id="location"
      className="scroll-mt-28 border-b border-white/8 py-20 md:py-28"
      aria-labelledby="location-title"
    >
      <div className="container">
        <motion.div
          variants={stagger}
          initial="hidden"
          whileInView="visible"
          viewport={VIEWPORT_ONCE}
          className="space-y-14 md:space-y-16"
        >
          <motion.div variants={fadeUp} className="max-w-3xl space-y-6">
            <p className="section-kicker">{location.kicker}</p>
            <h2
              id="location-title"
              className="font-heading text-3xl font-semibold text-white md:text-4xl"
            >
              {location.title}
            </h2>
            <p className="max-w-2xl text-base leading-8 text-white/58">
              {location.intro}
            </p>
          </motion.div>

          {location.offices.map((office, index) => {
            const maps = OFFICE_LOCATION_MAPS[index];
            if (!maps) return null;

            return (
              <motion.div
                key={office.name}
                variants={fadeUp}
                className="grid gap-10 lg:grid-cols-[minmax(0,0.42fr)_minmax(0,1fr)] lg:items-stretch lg:gap-14"
              >
                <div className="space-y-6">
                  <h3 className="font-heading text-xl font-semibold text-white md:text-2xl">
                    {office.name}
                  </h3>
                  <address className="not-italic">
                    <ul className="space-y-1 text-base leading-8 text-white/72">
                      {office.addressLines.map(line => (
                        <li key={line}>{line}</li>
                      ))}
                    </ul>
                  </address>
                  <a
                    href={maps.googleSearchUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-sm font-medium text-[#82c4ff] transition-colors hover:text-[#a8d4ff]"
                  >
                    {location.openMapsLabel}
                    <ExternalLink className="h-4 w-4" aria-hidden />
                  </a>
                </div>

                <div className="executive-panel overflow-hidden rounded-[1.25rem] p-0">
                  <div className="relative aspect-[4/3] w-full min-h-[220px] md:min-h-[260px] md:aspect-video lg:min-h-[300px] xl:min-h-[340px]">
                    <iframe
                      title={office.mapIframeTitle}
                      src={maps.embedUrl}
                      className="absolute inset-0 h-full w-full border-0"
                      loading="lazy"
                      referrerPolicy="no-referrer-when-downgrade"
                      allowFullScreen
                    />
                  </div>
                </div>
              </motion.div>
            );
          })}
        </motion.div>
      </div>
    </section>
  );
}
