...
function AppTermsHead({
  ...
  const canonical = canonicalAppTermsUrl(slug, language);
...
function AppTermsMain({ appId }: { appId: AppId }) {
  const { appTermsPages } = useDictionary();
  const { language } = useLanguage();
  const page = appTermsPages[appId];
...
export default function AppTerms() {
  ...
  const [, enParams] = useRoute("/:slug/terms");
  const [, deParams] = useRoute("/de/:slug/terms");
  ...
  return (
    <LanguageProvider defaultLanguage={defaultLanguage} key={path}>
      <AppTermsInner appId={catalog.id} slug={catalog.slug} />
    </LanguageProvider>
  );
}

function AppTermsInner({
  appId,
  slug,
}: {
  appId: AppId;
